	...
	if message in eventual_read.keys():
		if "get" in message:		
			...
			if len(eventual_read[message]) >= (model-2):
				...
				eventual_read[message].append("Received \"get 3 4\" from D, value is " + str(curr_val) + " Max delay is 0 s, system time is " + str(curr_time)) 
				max_time = -1
				best_message = ""
	...
	if data[0] != "D":
		...
		if "get" in message:
			if key in key_value:
				...
				server.send("SenD " + message + " value,time is " + val + " " + curr_time + " " + destination + "\n") 
		if "delete" in message:
			delete(key)
			server.send("SenD " + message + " " + destination + "\n")
		print data[20:len(data) - 2]		
