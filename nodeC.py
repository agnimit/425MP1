...
def received_eventual(data):
	...
	if data[0] != "C": #because we broadcasted message, we don't want to count a message sent from myself
		parsed = message.split(' ')
		key = int(parsed[1])		
		...
		if "get" in message: #eventual read request from another node
			if key in key_value:		
				...
				server.send("SenC " + message + " value: " + str(val) + " time: " + str(curr_time) + " " + destination + "\n") #we send timestamp back
...
def readData_server():
	...
	while 1:
		...
		if "received" in data.lower() and ("insert" in data or "get" in data or "update" in data or "delete" in data): #I sent the eventual request
			...
			if key in key_value.keys():
				message = "C has key " + str(key)
			else:
				message = "C does NOT have key " + str(key)
