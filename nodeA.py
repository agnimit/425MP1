...
def received_eventual(data):
	...
	if data[0] != "A": #because we broadcasted message, we don't want to count a message sent from myself
		parsed = message.split(' ')
		key = int(parsed[1])		
...
def readData_server():
	...
	while 1:
		...
		if "received" in data.lower() and ("insert" in data or "get" in data or "update" in data or "delete" in data): #I sent the eventual request
			...
			if key in key_value.keys():
				message = "A has key " + str(key)
			else:
				message = "A does NOT have key " + str(key)
